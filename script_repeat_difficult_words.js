...
difficult_words = JSON.parse(window.localStorage.getItem('difficult_words'));

for (let i = 0; i < difficult_words.length; i++) {
    for (let k = 0; k < 8; k++)
        for (let l = 0; l < 8; l++) {
            if (difficult_words[i].id == cards[k][l].id) {
                temp_cards.push({});
                temp_cards[i].id = cards[k][l].id;
                temp_cards[i].category_name = cards[k][l].category_name;
                temp_cards[i].word = cards[k][l].word;
                temp_cards[i].translation = cards[k][l].translation;
                temp_cards[i].image = cards[k][l].image;
                temp_cards[i].audioSrc = cards[k][l].audioSrc;
                temp_cards[i].statistics = cards[k][l].statistics;
...
for (let i = 0; i < difficult_words.length; i++) {
    ...
    FRONT.style.cssText = `background-image: url(${temp_cards[i].image});`;
    FRONT.id = temp_cards[i].audioSrc;
    CARD.append(FRONT);

    const CARD_HEADER = document.createElement('div');
    CARD_HEADER.classList.add('card_header');
    CARD_HEADER.innerHTML = temp_cards[i].word;
    FRONT.append(CARD_HEADER);

    const BACK = document.createElement('div');
    BACK.classList.add('back');
    BACK.style.cssText = `background-image: url(${temp_cards[i].image});`;
    CARD.append(BACK);

    const CARD_HEADER_1 = document.createElement('div');
    CARD_HEADER_1.classList.add('card_header');
    CARD_HEADER_1.innerHTML = temp_cards[i].translation;
...
function Game_Play_Current_Sound() {
    sound = temp_cards[Random_Array[Random_Array.length - 1]].audioSrc;
...
function Clear_After_Game() {
    ...
    for (let i = 1; i < difficult_words.length; i++) {
